                         ...
                         <div class="col-md-12 col-sm-12 col-xs-12">
                            ...
                            <form method="get" id="form_cust" action="simpan_cust">
                              <div class="col-md-12 col-sm-12 col-xs-12 tamma-bg" style="margin-bottom: 20px; padding-bottom:5px;padding-top:20px; ">
                                ...
                                <div class="col-md-4 col-sm-9 col-xs-12">
                                  <div class="form-group">
                                      <input type="text" class="form-control input-sm" readonly="true" name="id_cus" value="{{$id_cust}}">
                                      <input type="hidden" name="id_cus" value="{{$id_cust}}">
                                      <input type="hidden" name="id_cus_ut" value="{{ $maxid }}">
                                  </div>
                                </div>
                                <div class="col-md-2 col-sm-3 col-xs-12">
                              ...
                              <div align="right">
                                ...
                                <div class="form-group">
                                  <button type="button" name="edit_data" class="btn btn-warning hidden" onclick="edit()">Edit Data</button>
                                  <button type="button" name="update_data" class="btn btn-warning hidden" onclick="update()">Update Data</button>
                                  <button type="button" name="tambah_data" class="btn btn-danger" onclick="simpan()">Simpan Data</button>
                                </div> 
                              </div>
...
<script type="text/javascript">    
  ...
  function update(){
    ...
    $.ajax({
        url : baseUrl + "/master/datacust/cust_edit/cust_edit_proses/{{ $maxid }}",
        type:'get',
        data: a,
        dataType:'json',
        success:function(response){
          ...
          toastr.success(response.update_sukses, 'Sukses!');
         

          window.location = (baseUrl+'/master/datacust/tambah_cust')
  ...
  function edit(){
    ...
    $('#nama_cus').removeAttr('disabled');
    $('#email').removeAttr('disabled');
    $('#tipe_cust').removeAttr('disabled');
    $('#tgl_lahir').removeAttr('disabled');
    $('button[name="edit_data"]').addClass('hidden');
    $('button[name="update_data"]').removeClass('hidden');
    ...
    function simpan (){
      ...
      $.ajax({
        ...
        success:function(response){
          ...
          toastr.success(response.data, 'Sukses!');
          $('#no_hp').attr('disabled',true);
          $('#nama_cus').attr('disabled',true);
          $('#tgl_lahir').attr('disabled',true);
          $('#email').attr('disabled',true);
          $('#tipe_cust').attr('disabled',true);
          $('#alamat').attr('disabled',true);
         
          $('input[name="id_cus"]').val(response.id_cus);
          $('input[name="id_cus_ut"]').val(response.id_cus_ut);
  
          $('button[name="edit_data"]').removeClass('hidden');
          $('button[name="tambah_data"]').addClass('hidden');
